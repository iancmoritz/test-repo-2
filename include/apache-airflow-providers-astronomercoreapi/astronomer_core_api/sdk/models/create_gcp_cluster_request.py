...
class CreateGcpClusterRequest(BaseModel):
    """
    ...
    node_pools: Optional[conlist(CreateNodePoolRequest)] = Field(
        None, alias="nodePools"
    )
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in (
            ...
            raise ValueError(
                "must be one of enum values ('HOSTED', 'BRING_YOUR_OWN_CLOUD', 'VIRTUAL_RUNTIMES', 'SHARED')"
            )
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CreateGcpClusterRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CreateGcpClusterRequest:
        """Create an instance of CreateGcpClusterRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CreateGcpClusterRequest.parse_obj(obj)

        _obj = CreateGcpClusterRequest.parse_obj(
            {
                "db_instance_type": obj.get("dbInstanceType"),
                "is_dry_run": obj.get("isDryRun"),
                "k8s_tags": [
                    ClusterTag.from_dict(_item) for _item in obj.get("k8sTags")
                ]
                if obj.get("k8sTags") is not None
                else None,
                "name": obj.get("name"),
                "node_pools": [
                    CreateNodePoolRequest.from_dict(_item)
                    for _item in obj.get("nodePools")
                ]
