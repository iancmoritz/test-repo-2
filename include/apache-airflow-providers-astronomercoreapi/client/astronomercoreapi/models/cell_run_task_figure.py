...
class CellRunTaskFigure(BaseModel):
    """
    CellRunTaskFigure
    """
    ...
    __properties = ["data", "encoding", "format"]

    @validator('encoding')
    def encoding_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('base64'):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CellRunTaskFigure:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CellRunTaskFigure:
        """Create an instance of CellRunTaskFigure from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CellRunTaskFigure.parse_obj(obj)

        _obj = CellRunTaskFigure.parse_obj({
            "data": obj.get("data"),
