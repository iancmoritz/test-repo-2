...
class Runtime(BaseModel):
    ...
    __properties = ["apiCapabilities", "clusterId", "createdAt", "deletedAt", "description", "id", "name", "organizationId", "releaseName", "runtimeVersion", "type", "updatedAt", "workspaceId"]

    @validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('HYBRID', 'VIRTUAL', 'HOSTED_SHARED', 'HOSTED_DEDICATED'):
            raise ValueError("must be one of enum values ('HYBRID', 'VIRTUAL', 'HOSTED_SHARED', 'HOSTED_DEDICATED')")
        return value
    ...
    def from_dict(cls, obj: dict) -> Runtime:
        ...
        _obj = Runtime.parse_obj({
            "api_capabilities": RuntimeApiCapabilities.from_dict(obj.get("apiCapabilities")) if obj.get("apiCapabilities") is not None else None,
